...
| **3** | Hue color backend + swatches. Accent picker + Accent drill-in (both fixtures, DMX scenes). Scene fingerprint backend. Scene bar, room scene bar, consolidated More menu, macro buttons. | Parity with the old dashboard, plus color |
| **4** | Climate shadow state + cluster + drill-in with override form. Anthem state publishing + AV cluster + drill-in. Projector. Now Playing (+`SPOTIFY_NOW_PLAYING=1`). Utility rail (Door, Find My). Plex/Jellyfin chips. AC airflow animation. | Full `#t3` |
| **5** | Status screen (carries the admin links). Phone layout polish. | Feature-complete |
