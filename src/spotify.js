...
function spotifySwitchPlay(deviceToSwitch, debug_id) {
	...
	spotifyApi.refreshAccessToken()
		...
		console.log("%d - Refeshed Spotify Auth Token", debug_id);
		})
		...
		.then(function(data) {
			...
			(function transferAttempt(num) {
				if(data.body.is_playing) {
					...
					.then(function(data) {
						console.log("%d - Transfered playback to: %s", debug_id, deviceToSwitch);
					}, function(err) {
					...
					.then(function(data) {
						console.log("%d - Transfered Player & Started Playback on: %s", debug_id, deviceToSwitch);
					}, function(err) {
						if(num<transfer_attempts_delay.length && err.statusCode=="404"){
							var attempt_delay = transfer_attempts_delay[num];
							console.log('%d - Spotify device not yet active', debug_id);
